...
全アプリの設定タブは、最後のタブとして以下の順序で統一する
(アプリ固有の設定項目があれば①より前に置く)。

1. **GitHubへのバックアップ** — `renderBackupControls`が描画(見出し・注意書き・
   「今すぐ保存」「GitHubから復元」ボタン)
2. **GitHub連携** — `renderTokenSettings`が描画(トークン状態・入力欄・
   「トークンを保存」「今すぐ保存」ボタン)
3. **インポート・エクスポート** — 各アプリがローカルに描画する(オフラインでも
   動く必要があるため、共通モジュール経由にしない)。見出し・注意書き・
   ボタンラベル(「ファイルにエクスポート」「ファイルからインポート」)は
   全アプリで統一し、内部ロジック(ファイル名・マージ/置換の別)は
   アプリごとの実装でよい。

①②の描画に失敗した場合(オフライン等)は、それぞれのコンテナに
「GitHubバックアップ機能は現在利用できません(オフラインの可能性)。」と表示する。
